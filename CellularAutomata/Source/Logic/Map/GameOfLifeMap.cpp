...
std::array<int, static_cast<size_t>(GameOfLifeMap::TileStatus::kMaxTiles) > GameOfLifeMap::GetNeighborCounts(int index) const
{
...
void GameOfLifeMap::TileUpdater::UpdateOneThread(int startIndex, int endIndex)
{
	...
	while (m_jobStatus != JobStatus::kFinished)
	{
		...
		{
			auto neighbors = m_owner.GetNeighborCounts(i);

			if (m_currentTiles[i] == TileStatus::kAlive)
			{
				if (neighbors[static_cast<size_t>(TileStatus::kGrass)] < 2)
				{
					m_tileBuffer[i] = TileStatus::kDead;
				}
				else if (neighbors[static_cast<size_t>(TileStatus::kAlive)] > 3)
				{
					m_tileBuffer[i] = TileStatus::kDead;
				}
				else
			...
			else if (m_currentTiles[i] == TileStatus::kDead)
			{
				if (neighbors[static_cast<size_t>(TileStatus::kAlive)] >= 2 && neighbors[static_cast<size_t>(TileStatus::kGrass)] >= 1)
				{
					m_tileBuffer[i] = TileStatus::kAlive;
				}
				else if (neighbors[static_cast<size_t>(TileStatus::kGrass)] >= 2)
				{
					m_tileBuffer[i] = TileStatus::kGrass;
				}
				else
				{
					m_tileBuffer[i] = TileStatus::kDead;
			...
			else if (m_currentTiles[i] == TileStatus::kGrass)
			{
				if (neighbors[static_cast<size_t>(TileStatus::kAlive)] == 1 || neighbors[static_cast<size_t>(TileStatus::kAlive)] == 2)
				{
					m_tileBuffer[i] = TileStatus::kDead;
				}
				else if (neighbors[static_cast<size_t>(TileStatus::kAlive)] == 3)
